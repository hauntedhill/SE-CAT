...
public class ItemDAOTest {
	...
	public void getByCiretria() {
		...
		List<Item> result = dao.getItemBy(b1, true, null, null, null, null, f1);

		Assert.assertTrue(result.contains(item1));
		Assert.assertTrue(result.size() == 1);

		result = dao.getItemBy(b1, false, p2, e2, null, null, f2);

		Assert.assertTrue(result.contains(item2));
		Assert.assertTrue(result.size() == 1);

		result = dao.getItemBy(b1, false, p2, e2, "", "", f2);

		Assert.assertTrue(result.contains(item2));
		Assert.assertTrue(result.size() == 1);

		result = dao.getItemBy(b1, null, p2, e2, "", "", f2);

		Assert.assertTrue(result.contains(item2));
		Assert.assertTrue(result.size() == 1);

		result = dao.getItemBy(b1, true, p2, e2, "123", "123", f1);

		Assert.assertTrue(result.size() == 0);

		result = dao.getItemBy(b1, true, p2, e2, "123", "123", null);
